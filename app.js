...
async function fetchDataFromAPI() {
  try {
    const response = await fetch(
      "https://67185641b910c6a6e02bb95e.mockapi.io/users"
    );
...
async function deleteInfo(id) {
  if (confirm("Are you sure you want to delete?")) {
    try {
      const response = await fetch(
        `https://67185641b910c6a6e02bb95e.mockapi.io/users/${id}`,
        {
          method: "DELETE",
          headers: {
...
filterData.addEventListener("input", async () => {
  ...
  if (searchTerm !== "") {
    try {
      ...
      const response = await fetch(
        `https://67185641b910c6a6e02bb95e.mockapi.io/users?search=${searchTerm}`
      );
      ...
      if (filteredData != "Not found") {
        ...
        userInfo.innerHTML = `<tr class="employeeDetails"><td class="empty" colspan="11" align="center">No data available with the search term "${searchTerm}"</td></tr>`;
        table.style.minWidth = "1400px";
      }
    } catch (error) {
...
async function sortTable(column) {
  ...
  if (currentSort.direction === "default") {
    ...
    try {
      const response = await fetch(
        `https://67185641b910c6a6e02bb95e.mockapi.io/users?sortBy=${column}&order=${currentSort.direction}`
      );
      if (!response.ok) {
